...
var Bytes = class {
  ...
  commentedString(all = false) {
    ...
    let indent = (_a = this.indents[0]) != null ? _a : 0;
...
function parseServerHello(hello, sessionId) {
  ...
  hello.expectUint8(2, 0);
  const [endServerHello] = hello.expectLengthUint24(0);
  hello.expectUint16(771, 0);
  ...
  hello.expectUint8(sessionId.length, 0);
  hello.expectBytes(sessionId, 0);
  hello.expectUint16(4865, 0);
  hello.expectUint8(0, 0);
  const [endExtensions, extensionsRemaining] = hello.expectLengthUint16(0);
  while (extensionsRemaining() > 0) {
    const extensionType = hello.readUint16(0);
    const [endExtension] = hello.expectLengthUint16(0);
    if (extensionType === 43) {
      hello.expectUint16(772, 0);
      tlsVersionSpecified = true;
    } else if (extensionType === 51) {
      hello.expectUint16(23, 0);
...
var Crypter = class {
  constructor(mode, key, initialIv) {
    ...
    __publicField(this, "initialIv");
    __publicField(this, "ivLength");
    __publicField(this, "currentIv");
    __publicField(this, "currentIvDataView");
    __publicField(this, "initialIvLast32");
    __publicField(this, "recordsDecrypted", 0);
    this.mode = mode;
    this.key = key;
...
function readSeqOfSetOfSeq(cb, seqType) {
  ...
  while (seqRemaining() > 0) {
    ...
    const itemName = (_a = DNOIDMap[itemOID]) != null ? _a : itemOID;
...
var Cert = class {
  constructor(certData) {
    ...
    cb.expectUint8(universalTypeUTCTime, 0);
      ...
      if (extOID === "2.5.29.17") {
        ...
        while (authKeyIdSeqRemaining() > 0) {
          ...
          if (authKeyIdDatumType === (contextSpecificType | 0)) {
            ...
            const [endAuthKeyIdExtra, authKeyIdExtraRemaining] = cb.expectASN1Length(0);
            cb.skip(authKeyIdExtraRemaining(), 0);
            endAuthKeyIdExtra();
          } else {
  ...
  subjectAltNamesMatch(host) {
    var _a;
    const twoDotRegex = /[.][^.]+[.][^.]+$/;
    return ((_a = this.subjectAltNames) != null ? _a : []).some((cert) => {
      let certName = cert;
      let hostName = host;
...
async function parseEncryptedHandshake(host, record, serverSecret, hellos) {
  ...
  hs.expectUint16(1027, 0);
  const [endSignature] = hs.expectLengthUint16();
  hs.expectUint8(constructedUniversalTypeSequence, 0);
  const [endSigDer] = hs.expectASN1Length(0);
  hs.expectUint8(universalTypeInteger, 0);
  const [endSigRBytes, sigRBytesRemaining] = hs.expectASN1Length(0);
  let sigR = hs.readBytes(sigRBytesRemaining());
  endSigRBytes();
  hs.expectUint8(universalTypeInteger, 0);
  const [endSigSBytes, sigSBytesRemaining] = hs.expectASN1Length(0);
...
async function startTls(host, read, write) {
  ...
  clientFinishedRecord.writeUint8(22 /* Handshake */, 0);
...
\r
`);
  requestDataRecord.writeUint8(23 /* Application */, 0);
