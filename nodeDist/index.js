...
var init_stableStringify = __esm({
  "src/util/stableStringify.ts"() {
    "use strict";
  }
});
...
async function postgres(urlStr, transportFactory, neonPasswordPipelining = true) {
  const t0 = Date.now();
...
async function wsTransport(host, port, close = () => {
  ...
  const ws2 = await new Promise((resolve) => {
    const ws3 = new WebSocket(`wss://subtls-wsproxy.jawj.workers.dev/?address=${host}:${port}`);
...
if (dbUrl) {
  ...
  for (let i = 0; i < iterations; i++) {
    const html = await https2("https://bytebybyte.dev", "GET", tcpTransport2);
    if (i < iterations - 1)
