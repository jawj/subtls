...
r[e]=t;var u=(r,e,t)=>re(r,typeof e!="symbol"?e+"":e,t);function U(...r){if(r.length===1&&r[0]instanceof Uint8Array)return r[0];let e=r.
...
this.length),this.length+=e.length}getData(){return this.data.subarray(0,this.length)}};var at="\xB7\xB7 ";var Ht=new TextEncoder,se=new TextDecoder,D=class{constructor(e){u(this,"offset");
...
void 0?at.repeat(this.indents[0]):"",i=this.indents[0]??0,n=e?this.data.length:this.
...
${at.repeat(i)}`)}return t}};function bt(r,e,t,i=!0){let n=new D(1024);n.writeUint8(22,0),n.writeUint16(769,0);
let a=n.writeLengthUint16("TLS record");n.writeUint8(1,0);let c=n.writeLengthUint24();
...
let P=n.writeLengthUint16(0);return n.writeBytes(e),P(),$(),K(),y(),c(),a(),n}function z(r){return new Uint8Array(Array.from(r.matchAll(/[0-9a-f]/g)).map(e=>parseInt(
...
a]=r.expectLengthUint24(0);r.expectUint16(771,0);let c=r.readBytes(32);if(Z(c,[207,
...
`Unexpected extension 0x${f([h])}`);y()}if(s(),a(),n(),i!==!0)throw new Error("N\
o TLS version provided");if(t===void 0)throw new Error("No key provided");return t}var Oe=new RegExp(`  .+|^(${at})+`,"gm");var ct=16384,ae=ct+1+255;async function mt(r,e,t=ct){let n=await r(5);if(n===void 0)
return;if(n.length<5)throw new Error("TLS record header truncated");let a=new D(
...
headerData:n,header:a,type:c,length:s,content:o}}async function gt(r,e,t){let i=await mt(
r,23,ae);if(i===void 0)return;let n=new D(i.content),[a]=n.expectLength(n.remaining());
...
"0")} (expected 0x${t.toString(16).padStart(2,"0")})`);return h}}async function ce(r,e,t){
...
d.length-16)),h(),o.array()}async function Ct(r,e,t){let i=Math.ceil(r.length/ct),
n=[];for(let a=0;a<i;a++){let c=r.subarray(a*ct,(a+1)*ct),s=await ce(c,e,t);n.push(
...
"HMAC",i,e));return n}async function oe(r,e,t,i){let n=i>>3,a=Math.ceil(t/n),c=new Uint8Array(
a*n),s=await g.importKey("raw",r,{name:"HMAC",hash:{name:`SHA-${i}`}},!1,["sign"]),
o=new Uint8Array(0);for(let h=0;h<a;h++){let y=U(o,e,[h+1]),d=await g.sign("HMAC",
s,y),m=new Uint8Array(d);c.set(m,n*h),o=m}return c.subarray(0,t)}var qt=$t.encode(
"tls13 ");async function I(r,e,t,i,n){let a=$t.encode(e),c=U([(i&65280)>>8,i&255],
[qt.length+a.length],qt,a,[t.length],t);return oe(r,c,i,n)}async function Pt(r,e,t,i,n){let a=i>>>3,c=new Uint8Array(a),s=await g.importKey(
...
y),m=await ut(new Uint8Array(1),c,i),l=await g.digest(`SHA-${i}`,new Uint8Array(
0)),A=new Uint8Array(l),C=await I(m,"derived",A,a,i),v=await ut(C,h,i),p=await I(
v,"c hs traffic",d,a,i),b=await I(v,"s hs traffic",d,a,i),K=await I(p,"key",new Uint8Array(
...
async function Ot(r,e,t,i){let n=t>>>3,a=new Uint8Array(n),c=await g.digest(`SHA\
-${t}`,new Uint8Array(0)),s=new Uint8Array(c),o=await I(r,"derived",s,n,t),h=await ut(
o,a,t),y=await I(h,"c ap traffic",e,n,t),d=await I(h,"s ap traffic",e,n,t),m=await I(
...
c=BigInt(a.length),s=c-1n;for(let m=0n;m<c;m++){let l=n>>(m<<3n);if(l===0n)break;
a[Number(s-m)]^=Number(l&0xffn)}let o=t<<3,h={name:"AES-GCM",iv:a,tagLength:o,additionalData:i},
...
43?62:r===47?63:r===61?64:he(r)}function vt(r,e=de,t=!0){let i=r.length;t&&(r+="=".repeat(i%4));let n=0,a=0,c=64,
s=64,o=64,h=64,y=new Uint8Array(i*.75);for(;n<i;)c=e(r.charCodeAt(n++)),s=e(r.charCodeAt(
n++)),o=e(r.charCodeAt(n++)),h=e(r.charCodeAt(n++)),y[a++]=c<<2|s>>4,y[a++]=(s&15)<<
4|o>>2,y[a++]=(o&3)<<6|h;let d=s===64?0:o===64?2:h===64?1:0;return y.subarray(0,
a-d)}var M=class extends D{readASN1Length(e){let t=this.readUint8();if(t<128)return t;
...
expectASN1Length(0),n=this.readUint8(),a=`${Math.floor(n/40)}.${n%40}`;for(;i()>
0;){let c=0;for(;;){let s=this.readUint8();if(c<<=7,c+=s&127,s<128)break}a+=`.${c}`}
return t(),a}readASN1Boolean(e){let[t,i]=this.expectASN1Length(0),n=i();if(n!==1)
throw new Error(`Boolean has weird length: ${n}`);let a=this.readUint8(),c;if(a===
255)c=!0;else if(a===0)c=!1;else throw new Error(`Boolean has weird value: 0x${f(
[a])}`);return t(),c}readASN1UTCTime(){let[e,t]=this.expectASN1Length(0),n=this.
...
50?1900:2e3),l=new Date(`${m}-${c}-${s}T${o}:${h}:${y}Z`);return e(),l}readASN1GeneralizedTime(){
let[e,t]=this.expectASN1Length(0),n=this.readUTF8String(t()).match(/^([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})?([0-9]{2})?([.][0-9]+)?(Z)?([-+][0-9]+)?$/);
if(!n)throw new Error("Unrecognised ASN.1 generalized time format");let[,a,c,s,o,
...
"Invalid ASN.1 generalized time format (Z and timezone)");let A=new Date(`${a}-${c}\
...
1;s>0;s--)a[s]=255&a[s-1]<<c|a[s]>>>i;a[0]=a[0]>>>i}return e(),a}};var ft=1,it=2,N=48,le=49,et=6,ye=19,me=20,ge=12,ue=22,kt=23,Lt=24,pt=5,G=4,xt=3,
...
0);for(;n()>0;){r.expectUint8(le,0);let[a]=r.expectASN1Length(0);r.expectUint8(N,
0);let[c]=r.expectASN1Length(0);r.expectUint8(et,0);let s=r.readASN1OID(),o=fe[s]??
...
0),m=r.readUTF8String(d());y(),c(),a();let l=t[o];l===void 0?t[o]=m:typeof l=="s\
...
i]=t.expectASN1Length(0),n=t.offset;t.expectUint8(N,0);let[a]=t.expectASN1Length(
0);t.expectBytes([160,3,2,1,2],0),t.expectUint8(it,0);let[c,s]=t.expectASN1Length(
0);this.serialNumber=t.subarray(s()),c(),t.expectUint8(N,0);let[o,h]=t.expectASN1Length(
...
ca:T,pathLength:B}}else t.skip(Y(),0);J()}O(),P(),a(),this.signedData=t.data.subarray(
...
let t=/[.][^.]+[.][^.]+$/;return(this.subjectAltNames??[]).find(i=>{let n=i,a=e;
if(t.test(e)&&t.test(n)&&n.startsWith("*.")&&(n=n.slice(1),a=a.slice(a.indexOf("\
."))),n===a)return!0})}isValidAtMoment(e=new Date){return e>=this.validityPeriod.
...
BEGIN ${t}-----([a-zA-Z0-9=+\\/\\n\\r]+)-----END ${t}-----`,"g"),n=[],a=null;for(;a=
i.exec(e);){let c=a[1].replace(/[\r\n]/g,""),s=vt(c);n.push(s)}return n}static fromPEM(e){
return this.uint8ArraysFromPEM(e).map(t=>new this(t))}},Q=class extends rt{static databaseFromPEM(e){
let t=this.uint8ArraysFromPEM(e),i=[0],n={},a=new yt;for(let s of t){let o=new this(
s),h=i.length-1;o.subjectKeyIdentifier&&(n[f(o.subjectKeyIdentifier)]=h),n[this.
stringFromDistinguishedName(o.subject)]=h,a.append(s),i[i.length]=i[h]+s.length}
let c=a.getData();return{index:{offsets:i,subjects:n},data:c}}static findInDatabase(e,t){
let{index:{subjects:i,offsets:n},data:a}=t,c=typeof e=="string"?e:rt.stringFromDistinguishedName(
e),s=i[c];if(s===void 0)return;let o=n[s],h=n[s+1],y=a.subarray(o,h);return new this(
y)}};async function St(r,e,t,i,n){r.expectUint8(N,0);let[a]=r.expectASN1Length(0);r.expectUint8(
it,0);let[c,s]=r.expectASN1Length(0),o=r.readBytes(s());c(),r.expectUint8(it,0);
let[h,y]=r.expectASN1Length(0),d=r.readBytes(y());h(),a();let m=(p,b)=>p.length>
...
"ECDSA-SECP256R1-SHA256 certificate verify failed")}async function zt(r,e,t,i=!0,n=!0){for(let h of e);let a=e[0];if(a.subjectAltNameMatchingHost(
...
pported signing algorithm");if(A){o=!0;break}}return o}var xe=new TextEncoder;async function Gt(r,e,t,i,n,a=!0,c=!0){let s=new M(await e());
...
"Invalid server verify hash");if(!await zt(r,v,n,a,c))throw new Error("Validated\
 certificate chain did not end in a trusted root");return[s.data,d]}async function Se(r,e,t,i,{useSNI:n,requireServerTlsExtKeyUsage:a,requireDigitalSigKeyUsage:c,
writePreData:s,expectPreData:o,commentPreData:h}={}){n??(n=!0),a??(a=!0),c??(c=!0),
...
X,e,a,c),w=new D(6);w.writeUint8(20,0),w.writeUint16(771,0);let T=w.writeLengthUint16();
...
this.outstandingRequest=void 0;let n=this.queue[0],a=n.length;if(a===t)return this.
queue.shift(),e(n);if(a>t)return this.queue[0]=n.subarray(t),e(n.subarray(0,t));
{let c=new Uint8Array(t),s=t,o=0;for(;s>0;){let h=this.queue[0],y=h.length;y<=s?
(this.queue.shift(),c.set(h,o),o+=y,s-=y):(this.queue[0]=h.subarray(s),c.set(h.subarray(
0,s),o),s-=s,o+=s)}return e(c)}}bytesInQueue(){return this.queue.reduce((e,t)=>e+
...
let{socket:t}=this,{readyState:i}=t;return i==="opening"||i==="open"}};function Qt(r,e=(i,n)=>n,t){return JSON.stringify(r,(n,a)=>e(n,typeof a!="object"||
a===null||Array.isArray(a)?a:Object.fromEntries(Object.entries(a).sort(([c],[s])=>c<
s?-1:c>s?1:0))),t)}export{Et as SocketReadQueue,Q as TrustedCert,Rt as WebSocketReadQueue,vt as base64Decode,
